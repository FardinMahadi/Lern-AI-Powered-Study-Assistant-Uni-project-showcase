...
const getMousePos = (e: any, container: any): { x: number; y: number } => {
  if (container) {
...
const Crosshair = ({ color = "white", containerRef = null }: any) => {
  ...
  useEffect(() => {
    ...
    const tl = gsap
      .timeline({
        ...
        onComplete: () => {
          if (lineHorizontalRef.current && lineVerticalRef.current) {
            (lineHorizontalRef.current as any).style.filter = (
              lineVerticalRef.current as any
            ).style.filter = "none";
          }
        },
    ...
    const render = () => {
      ...
      for (const key in renderedStyles) {
        const k = key as keyof typeof renderedStyles;
